...
public class GetGroupsPagedHandler : IRequestHandler<GetGroupsPagedQuery, PagedResult<GroupViewModel>>
{
    ...
    public async Task<PagedResult<GroupViewModel>> Handle(GetGroupsPagedQuery query, CancellationToken ct)
    {
        ...
        var cte = @"WITH paged AS MATERIALIZED 
                (SELECT g.id,g.name,g.teacher_id AS TeacherId, t.first_name AS TeacherFirstName, t.last_name AS TeacherLastName,
                g.subject_id AS SubjectId, s.name AS SubjectName, g.is_active AS IsActive,g.max_number_of_classes AS MaxNumberOfClasses,
                g.number_of_classes_left AS NumberOfClassesLeft, g.created_at AS CreatedAt, g.updated_at AS UpdatedAt, StudentIds
        ...
        var users = (await multi.ReadAsync<GroupViewModel>()).ToList();
